...
**用户事实类（user_fact）：**
- **user_profile**：用户画像（身份+属性） — "用户叫李明"、"用户生日是1月1日"、"用户身高180cm"
...
**情感记忆类（emotional）：**
- **emotional_state**：情感状态（情绪+倾向） — "用户今天心情很好"、"用户对这部电影感到失望"
...
## 输出格式
JSON 数组，每个元素包含：
- "memory_type": 细粒度类型（见上方分类，如 "user_profile"、"user_preference"、"episodic_event" 等）
- "source": "ai_related" | "user_related" | "other"
- "content": 记忆文本（简洁、完整的句子，第三人称）
- "importance": 重要性评分（0.0~1.0）
  - user_profile/user_relationship > 0.8
...
class MemoryExtractor:
    ...
    def _extract_by_llm(
        ...
        for item in items:
            ...
            _VALID_TYPES = {
                # 用户事实
                "user_fact", "user_profile", "user_preference",
                ...
                "summary",
                # 向后兼容旧分类名称（V1.2-14）
                "user_identity", "user_attribute", "emotional_mood", "emotional_sentiment",
            }
            ...
            if mem_type in _LEGACY_TYPE_MAP:
                mem_type = _LEGACY_TYPE_MAP[mem_type]
