...
class CardManager:
    ...
    def __init__(self, cards_dir: str = "./cards"):
        # 安全检查：防止路径遍历攻击
        input_path = Path(cards_dir)
        project_root = Path(__file__).parent.parent.resolve()
        
        if not input_path.is_absolute():
            # 相对路径：相对于项目根目录，但需要确保不会跳出
            candidate_path = (project_root / cards_dir).resolve()
        else:
            # 绝对路径：也需要确保安全
            candidate_path = input_path.resolve()
        
        # 关键安全检查：确保最终路径在项目根目录内
        if not str(candidate_path).startswith(str(project_root)):
            # 如果不在项目内，强制使用项目内的目录
            logger.warning(f"检测到可能的路径遍历尝试，强制使用项目内目录: {cards_dir}")
            self.cards_dir = project_root / "cards"
        else:
            self.cards_dir = candidate_path
