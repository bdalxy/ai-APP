...
try:
    ...
    psutil = None
    logger.warning("psutil 模块未安装，某些设备信息检测功能将不可用")
    ...
    def _get_os_version(self) -> str:
        ...
        if self._platform == 'windows':
            ...
            try:
                ...
                logger.debug("/etc/os-release 文件不存在")
            except PermissionError:
                logger.debug("无法读取 /etc/os-release，权限不足")
            except Exception as e:
                logger.debug(f"读取操作系统版本失败: {e}")
    ...
    def _get_device_model(self) -> str:
        ...
        if self._platform == 'windows':
            try:
                ...
                logger.debug("Windows 注册表键不存在")
            except PermissionError:
                logger.debug("无法访问 Windows 注册表，权限不足")
            except Exception as e:
                logger.debug(f"读取 Windows 设备型号失败: {e}")
            return "Windows PC"
        elif self._platform == 'darwin':
            try:
                import subprocess
                ...
                logger.debug("sysctl 命令不存在")
            except subprocess.CalledProcessError as e:
                logger.debug(f"sysctl 命令执行失败: {e}")
            except Exception as e:
                logger.debug(f"读取 macOS 设备型号失败: {e}")
            return "Mac"
        elif self._platform == 'linux':
            try:
                ...
                logger.debug("/proc/device-tree/model 文件不存在")
            except PermissionError:
                logger.debug("无法读取 /proc/device-tree/model，权限不足")
            except Exception as e:
                logger.debug(f"读取 Linux 设备型号失败(方法1): {e}")
                
            try:
                with open('/sys/devices/virtual/dmi/id/product_name', 'r') as f:
                    return f.read().strip()
            except FileNotFoundError:
                logger.debug("/sys/devices/virtual/dmi/id/product_name 文件不存在")
            except PermissionError:
                logger.debug("无法读取 /sys/devices/virtual/dmi/id/product_name，权限不足")
            except Exception as e:
                logger.debug(f"读取 Linux 设备型号失败(方法2): {e}")
    ...
    def _get_cpu_cores(self) -> int:
        ...
        if psutil:
            try:
                ...
                logger.debug(f"使用 psutil 获取 CPU 核心数失败: {e}")
        try:
            return os.cpu_count() or 4
        except Exception as e:
            logger.debug(f"获取 CPU 核心数失败: {e}")
            return 4
    ...
    def _get_total_memory(self) -> float:
        ...
        if psutil:
            try:
                ...
                logger.debug(f"使用 psutil 获取总内存失败: {e}")
        return 4.0  # 默认值
    ...
    def _get_available_memory(self) -> float:
        ...
        if psutil:
            try:
                ...
                logger.debug(f"使用 psutil 获取可用内存失败: {e}")
        return 2.0  # 默认值
    ...
    def _get_disk_space(self) -> float:
        ...
        if psutil:
            try:
                ...
                logger.debug("无法读取磁盘空间，权限不足")
            except Exception as e:
                logger.debug(f"使用 psutil 获取磁盘空间失败: {e}")
        return 64.0  # 默认值
