...
HALF_LIFE_DAYS: dict[str, float] = {"episodic": 7.0, "semantic": 30.0, "user_fact": 180.0}
DEFAULT_HALF_LIFE: float = 30.0
TYPE_MULTIPLIERS: dict[str, float] = {"episodic": 0.8, "semantic": 1.0, "user_fact": 1.2}
...
def get_weight(entry: MemoryEntry, current_time: datetime | None = None) -> float:
    ...
    return entry.importance * decay * type_mult
...
def _parse_iso_datetime(iso_str: str) -> datetime:
    for fmt in ["%Y-%m-%dT%H:%M:%S.%f%z", "%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%S.%f", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S"]:
        try:
            ...
            continue
    raise ValueError(f"无法解析时间: {iso_str}")
